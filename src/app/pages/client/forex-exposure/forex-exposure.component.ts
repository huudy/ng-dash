...
@Component({
  selector: 'app-forex-exposure',
  templateUrl: './forex-exposure.component.html',
  styleUrls: ['./forex-exposure.component.scss']
})
export class ForexExposureComponent implements OnInit, OnChanges {
  ...
  buildtable() {
    ...
    this.table = this.DatatablesService.createDatatable(
      '#foreignExchangeTransactionsTable',
      {
        data: self.data,
        columns: [
          { data: "currencyPair", render: function (data, type, row) { return self.readableCurrencyPair(row['currencyPair'], row['boughtSold']) } },
          ...
          { data: "weightedTradeRate", className: "right-align" },
          { data: null, defaultContent: '' },
          { data: "boughtSold", visible: false }
        ],
        createdRow: function (row, data) {
          $('td:eq(0)', row).html(self.readableCurrencyPair(data['currencyPair'], data['boughtSold']));

          let allGraphData = JSON.parse(data['data']);
          for (let key in allGraphData) {
            let content = $(self.buildBadge(self.DatatablesService.addTooltip, 'by ' + key, '#foreignExchangeTransactionsTable'))
            ...
            $('td:eq(5)', row).append(content);
          }

        }
      },
      self.drawGraph,
      self.DatatablesService.configureExport([0, 1, 2, 3, 4])
    );
  }
