...
@Component({
  selector: 'app-myonemarkets-table',
  templateUrl: './myonemarkets-table.component.html',
  styleUrls: ['./myonemarkets-table.component.scss']
})
export class MyonemarketsTableComponent implements OnInit, OnChanges {
  ...
  buildTable() {
    ...
    this.table = this.DatatablesService.createDatatable(
      '#issuancesmyonemarketsDetailsTable',
      {
        data: data,
        columns: [
        ...
        createdRow: function (row, data) {
          $('td:eq(4)', row).html(self.DatatablesService.formatAmount(data, 'issueVolume'));
          ...
          $('td:eq(8)', row).html(self.productDetails(data, detailFields, detailFieldsNames));
        }
      }
    );
  }
